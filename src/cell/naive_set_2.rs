...
impl NaiveSet2Cell {
    fn new(meta: model::Meta, client: Arc<Platform>, outdated_gc_sec: u64) -> NaiveSet2Cell {
        NaiveSet2Cell {
            meta,
            client,
            outdated_gc_sec,
...
pub struct NaiveSet2CellFactory;

impl CellFactory<NaiveSet2Cell> for NaiveSet2CellFactory {
    fn new(&self, meta: model::Meta, client: Arc<Platform>) -> Arc<NaiveSet2Cell> {
        let outdated_gc_sec = if meta.key.ends_with("1") {
            OUTDATED_SLOT_GC_SEC_1
        } else {
            OUTDATED_SLOT_GC_SEC_2
        };
        let cell = Arc::new(NaiveSet2Cell::new(meta, client, outdated_gc_sec));
